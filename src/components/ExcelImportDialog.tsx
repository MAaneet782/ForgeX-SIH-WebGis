...
const ExcelImportDialog = ({ isOpen, onOpenChange, claims }: ExcelImportDialogProps) => {
  ...
  const handleImport = async () => {
    ...
    // Use the user-facing 'id' for uniqueness check from existing claims
    const existingClaimUserFacingIds = new Set(claims.map(c => c.id));
    const generatedUserFacingIdsInBatch = new Set<string>();

    const generateUniqueUserFacingClaimId = (baseId?: string): string => {
      let newId = baseId || `C-${Math.random().toString(36).substring(2, 9).toUpperCase()}`; // Generate a random string for user-facing ID
      let counter = 0;
      while (existingClaimUserFacingIds.has(newId) || generatedUserFacingIdsInBatch.has(newId)) {
        counter++;
        newId = `C-${Math.random().toString(36).substring(2, 9).toUpperCase()}`; // Regenerate if collision
        if (counter > 100) { 
            console.warn("Too many ID collisions, falling back to a more robust random ID.");
            newId = `C-${crypto.randomUUID().substring(0, 8).toUpperCase()}`; // Fallback to part of UUID
            break;
        }
      }
      generatedUserFacingIdsInBatch.add(newId);
      return newId;
    };
    ...
    const newClaims = jsonData
      ...
      .map(row => {
        ...
        const waterAvailability: Claim['waterAvailability'] = waterAvailabilities.includes(waterAvailabilityRaw) ? waterAvailabilities[waterAvailabilities.indexOf(waterAvailabilityRaw)] || waterAvailabilities[Math.floor(Math.random() * waterAvailabilities.length)] : waterAvailabilities[Math.floor(Math.random() * waterAvailabilities.length)];
        ...
        return {
          // dbId will be auto-generated by Supabase
          claim_id: finalUserFacingClaimId, // This is the user-facing text ID
